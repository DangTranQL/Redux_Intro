...
const RootComponent = (props) => {
  ...
  const removeProductFromCart = ({product}) => {
    const id = product.id;
    for (let i = 0; i < cart.products.length; i++) {
      if ((cart.products[i].id === id)&&(cart.products[i].qty >0)){
        cart.products[i].qty -= 1;
        cart.products[i].price -= product.price;
        cart.totalPrice -= product.price;
      }
  ...
  return (
    <WrapperBox>
      ...
      <Grid container spacing={2} p="1rem">
        <Grid item md={6}>
          <ProductPage products={products} addProduct={addProductCart} removeProduct={removeProductFromCart}/>
        </Grid>
        <Grid item md={6}>
          <CartPage cart={cart} />
        </Grid>
      ...
      <Grid container justifyContent="center">
        ...
        <Grid item xs={8} >
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            <Button variant="success" sx={{ width: "5rem" }} onClick={() => props.addProduct(props)} >
              Add
            </Button>
            <Button variant="error" sx={{ width: "5rem" }} onClick={() => props.removeProduct(props)}>
              Remove
            </Button>
          </div>
        </Grid>
  ...
  return (
    <WrapperBox>
      ...
      <Grid container justifyContent="center">
        ...
        <Grid item xs={8} >
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            <Button variant="success" size="sm" style={{ width: "5rem" }} onClick={() => props.addProduct(props)}>
              Add
            </Button>
            <Button variant="error" size="sm" style={{ width: "5rem" }} onClick={() => props.removeProduct(props)}>
              Remove
            </Button>
          </div>
        </Grid>
